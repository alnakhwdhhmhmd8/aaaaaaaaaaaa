...
@app.on_message(filters.command(["ماندو"], "") & filters.group, group=826372700036)
...
@app.on_message(filters.command(["ꜱᴏʜᴀɪʟᴀ","سوهيلا","سهيله","سوسو"], "") & filters.group, group=8263727036)
async def handle_user_info(client, message):
...
async def handle_user_info(client, message):
    user = await client.get_chat("M_D_O_2")
    msg = await message.reply("😈")
    await sleep(2)
    await msg.delete()
    name = user.first_name
...
async def handle_user_info(client, message):
    user = await client.get_chat("xvwwvl")
    name = user.first_name
    if user.photo:
        photo = await app.download_media(user.photo.big_file_id)
        await message.reply_photo(photo, caption=f"<blockquote><b>╭◉ᚐɴᴧᴍᴇ : {name}\n│᚜◉ᚐᴜsᴇꝛ : @{user.username}\n│᚜◉ᚐɪᴅ : <code>{user.id}</code>\n╰◉ᚐʙɪᴏ : {user.bio}</b></blockquote>", 
...
@app.on_message(filters.command(["كيمي","مطور الشيخ","صاحب سورس الشيخ","زيرو","مطور السورس","ᴋɪᴍᴍʏ","KIM"], ""), group=11)
